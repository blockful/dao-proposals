...
The master stream is \$3.09M/yr: the cohort (\$1.69M) plus eth.limo and Blockful (\$700k each), whose SPP2 two-year
streams run into next year. The pod switch turns off the SPP2 providers that were not renewed (NameHash \$1.1M, EFP
\$500k, ZK Email \$400k, JustaName \$300k) in the same batch that starts the new ones.
...
\$4.5M while only taking in \$3.09M, about -\$1.41M/yr (\$3,869/day). The executable covers this by sending a \$250k
margin to the pod up front, which carries it about 65 days past execution.
