 ...
 * @dev Live:
 * https://www.tally.xyz/gov/ens/proposal/37471031935121384443990643698532486694478877174074595786824358199918042537243
 *
...
contract Proposal_ENS_EP_6_38_Test is ENS_Governance, SafeHelper, ZodiacRolesHelper {
    ...
    function _beforeProposal() public override {
        ...
        (bool ok, bytes memory ret) =
            address(endowmentSafe).staticcall(abi.encodeWithSignature("isModuleEnabled(address)", ROLES_V1));
        ...
        _safeExecuteTransaction(ROCKET_POOL_DEPOSIT_V3, abi.encodeWithSelector(DEPOSIT_SELECTOR));
        vm.stopPrank();

        // SPK transfer currently works
        vm.startPrank(karpatkey);
        _safeExecuteTransaction(SPK, abi.encodeWithSelector(TRANSFER_SELECTOR, address(timelock), uint256(1)));
        vm.stopPrank();
    ...
    function _afterExecution() public override {
        ...
        vm.startPrank(karpatkey);
        _safeExecuteTransaction(GHO, abi.encodeWithSelector(APPROVE_SELECTOR, F_GHO, uint256(1e18)));
        vm.stopPrank();

        // GHO approve to GPv2VaultRelayer should be allowed
        vm.startPrank(karpatkey);
        _safeExecuteTransaction(GHO, abi.encodeWithSelector(APPROVE_SELECTOR, GPV2_VAULT_RELAYER, uint256(1e18)));
        vm.stopPrank();
        ...
        vm.startPrank(karpatkey);
        _safeExecuteTransaction(FLUID, abi.encodeWithSelector(APPROVE_SELECTOR, GPV2_VAULT_RELAYER, uint256(1e18)));
        vm.stopPrank();
        ...
        vm.startPrank(karpatkey);
        _safeExecuteTransaction(ROCKET_POOL_DEPOSIT_V4, abi.encodeWithSelector(DEPOSIT_SELECTOR));
        vm.stopPrank();
    ...
    function _buildRevocationTransactions() internal view returns (bytes memory) {
        return bytes.concat(
            ...
            _packTx(
                address(endowmentSafe), abi.encodeWithSignature("disableModule(address,address)", PREV_MODULE, ROLES_V1)
            ),
            ...
            _packTx(
                address(roles),
                abi.encodeWithSelector(
                    IRolesModifier.revokeFunction.selector, MANAGER_ROLE, ROCKET_POOL_DEPOSIT_V3, DEPOSIT_SELECTOR
                )
            ),
            ...
            _packTx(
                address(roles),
                abi.encodeWithSelector(
                    IRolesModifier.revokeFunction.selector, MANAGER_ROLE, SPARK_REWARDS, SPARK_CLAIM_SELECTOR
                )
            )
        );
    }
    ...
    function _buildAnnotationRemoval() internal view returns (bytes memory) {
        ...
        return _packTx(
            ANNOTATION_REGISTRY,
            abi.encodeWithSelector(IAnnotationRegistry.post.selector, jsonPayload, "ROLES_PERMISSION_ANNOTATION")
        );
    }
        ...
        return bytes.concat(
            ...
            _packTx(
                ...
                abi.encodeWithSelector(
                    ...
                    GHO,
                    APPROVE_SELECTOR,
                    _buildGhoApproveConditions(),
                    EXEC_NONE
                )
            ),
            // TX 11: scopeTarget FLUID
            _packTx(address(roles), abi.encodeWithSelector(IRolesModifier.scopeTarget.selector, MANAGER_ROLE, FLUID)),
            ...
            _packTx(
                address(roles), abi.encodeWithSelector(IRolesModifier.scopeTarget.selector, MANAGER_ROLE, FLUID_MERKLE)
            ),
            ...
            _packTx(
                ...
                abi.encodeWithSelector(
                    ...
                    FLUID_MERKLE,
                    FLUID_CLAIM_SELECTOR,
                    _buildFluidMerkleClaimConditions(),
                    EXEC_NONE
                )
            )
        );
