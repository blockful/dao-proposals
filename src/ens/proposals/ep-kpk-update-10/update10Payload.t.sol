...
/**
 * @title Endowment permissions to kpk — Update #10 (pre-draft, round 1: the permission payload)
 * @notice Pre-draft calldata review for
 *     https://discuss.ens.domains/t/draft-endowment-permissions-to-kpk-update-10/22323
 *
 * SUPERSEDED AS AN EXECUTABLE. kpk's revised execution (forum post 4, 2026-09-14) no longer
 * applies this payload to the live Roles Modifier; it swaps the Modifier for a pre-configured
 * v2.1.1 instance instead. This file is retained, pinned to its original fork block, because
 * it is the manual derivation of the Update #10 permission delta from the forum
 * specification. calldataCheck.t.sol (round 2) proves that the new Modifier equals the live
 * policy with exactly this delta applied. Both must stay green.
 *
