...
contract Proposal_UNI_93_UNIfication_Test is UNI_Governance {
    ...
    // Fee infrastructure
    IV3FeeAdapter public constant v3FeeAdapter = IV3FeeAdapter(0x5E74C9f42EEd283bFf3744fBD1889d398d40867d);
    ...
    IAgreementAnchor public constant hartLamburAgreement = IAgreementAnchor(0x33A56942Fe57f3697FE0fF52aB16cb0ba9b8eadd);
    ...
    bytes32 public constant agreementSchema = 0x504f10498bcdb19d4960412dbade6fa1530b8eed65c319f15cbe20fadafe56bd;
    ...
    function _beforeProposal() public override {
        ...
        assertEq(
            uniswapLabsAgreementSignaturePartyA, bytes32(0), "Uniswap Labs agreement signature party A should be 0"
        );

        address uniswapLabsAgreementPartyB = uniswapLabsAgreement.PARTY_B();
        assertEq(
            uniswapLabsAgreementPartyB, uniswapLabsWallet, "Uniswap Labs agreement party B should be uniswapLabsWallet"
        );
        bytes32 uniswapLabsAgreementSignaturePartyB = uniswapLabsAgreement.partyB_attestationUID();
        assertEq(
            uniswapLabsAgreementSignaturePartyB, bytes32(0), "Uniswap Labs agreement signature party B should be 0"
        );
        ...
        assertEq(
            hartLamburAgreementPartyB, hartLamburWallet, "Hart Lambur agreement party B should be hartLamburWallet"
        );

        bytes32 hartLamburAgreementSignaturePartyB = hartLamburAgreement.partyB_attestationUID();
        assertNotEq(
            hartLamburAgreementSignaturePartyB, bytes32(0), "Hart Lambur agreement signature party B should not be 0"
        );
        ...
        assertEq(
            daoJonesLlcAgreementSignaturePartyA, bytes32(0), "DAO Jones LLC agreement signature party A should be 0"
        );

        address daoJonesLlcAgreementPartyB = daoJonesLlcAgreement.PARTY_B();
        assertEq(
            daoJonesLlcAgreementPartyB, daoJonesLlcWallet, "DAO Jones LLC agreement party B should be daoJonesLlcWallet"
        );
        bytes32 daoJonesLlcAgreementSignaturePartyB = daoJonesLlcAgreement.partyB_attestationUID();
        assertEq(
            daoJonesLlcAgreementSignaturePartyB, bytes32(0), "DAO Jones LLC agreement signature party B should be 0"
        );
    ...
    function _buildAttestCalldata(address recipient, bytes32 documentHash) internal pure returns (bytes memory) {
        ...
        IEAS.AttestationRequest memory request = IEAS.AttestationRequest({ schema: agreementSchema, data: data });
    ...
    function _afterExecution() public override {
        ...
        assertEq(
            deadAddressBalanceAfter, deadAddressBalanceBefore + uniBurnAmount, "Dead address should receive 100M UNI"
        );
        ...
        bytes32 uniswapLabsAgreementSignaturePartyA = uniswapLabsAgreement.partyA_attestationUID();
        assertNotEq(
            uniswapLabsAgreementSignaturePartyA, bytes32(0), "Uniswap Labs agreement signature party A should not be 0"
        );

        bytes32 hartLamburAgreementSignaturePartyA = hartLamburAgreement.partyA_attestationUID();
        assertNotEq(
            hartLamburAgreementSignaturePartyA, bytes32(0), "Hart Lambur agreement signature party A should not be 0"
        );

        bytes32 daoJonesLlcAgreementSignaturePartyA = daoJonesLlcAgreement.partyA_attestationUID();
        assertNotEq(
            daoJonesLlcAgreementSignaturePartyA, bytes32(0), "DAO Jones LLC agreement signature party A should not be 0"
        );
