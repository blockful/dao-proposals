...
contract EncodeTally is Script {
    ...
    function _sig() internal pure returns (bytes memory) {
        return abi.encodePacked(bytes32(uint256(uint160(TIMELOCK))), bytes32(0), uint8(1));
    }
    ...
    function run() external view {
        ...
        bytes memory tx5 = abi.encodeWithSelector(
            ISafe.execTransaction.selector, ROLES_MOD, 0, inner5, uint8(0), 0, 0, 0, address(0), address(0), _sig()
        );
        ...
        conditions[1] = ConditionFlat({
            parent: 0, paramType: 1, operator: 16, compValue: abi.encodePacked(bytes32(uint256(uint160(TIMELOCK))))
        });
        conditions[2] = ConditionFlat({ parent: 0, paramType: 1, operator: 0, compValue: "" });
        bytes memory inner6 = abi.encodeWithSelector(
            IRolesModifier.scopeFunction.selector, MANAGER_ROLE, USDC, IERC20.transfer.selector, conditions, uint8(0)
        );
        bytes memory tx6 = abi.encodeWithSelector(
            ISafe.execTransaction.selector, ROLES_MOD, 0, inner6, uint8(0), 0, 0, 0, address(0), address(0), _sig()
        );
        ...
        bytes memory tx7 = abi.encodeWithSelector(
            ISafe.execTransaction.selector, ROLES_MOD, 0, inner7, uint8(0), 0, 0, 0, address(0), address(0), _sig()
        );
        ...
        bytes memory inner8 =
            abi.encodeWithSelector(IRolesModifier.allowFunction.selector, MANAGER_ROLE, TIMELOCK, bytes4(0), uint8(1));
        bytes memory tx8 = abi.encodeWithSelector(
            ISafe.execTransaction.selector, ROLES_MOD, 0, inner8, uint8(0), 0, 0, 0, address(0), address(0), _sig()
        );
